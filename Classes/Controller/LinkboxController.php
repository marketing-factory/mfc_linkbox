<?php
namespace Mfc\MfcLinkbox\Controller;
 ...
 */
class LinkboxController extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin
{
    ...
    protected function getTreeLevelUids()
    {
        ...
        $tree = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance(QueryGenerator::class);
        $uidListString = $tree->getTreeList($this->parentPid, 1, 0, 'hidden = 0 AND doktype in (1,7) AND nav_hide = 0');
        $uidListArray = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(',', $uidListString);
    ...
    protected function getLinkByPid($pid, $title = '', $wrap = '')
    {
        ...
        return $this->cObj->typoLink($title, $linkConf);
    }
}
